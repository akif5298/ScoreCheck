...
// `applyMapping` used to live here. It is pure, but this module builds the pg pool and the
// Supabase clients at import time (via `./supabase`), which meant any consumer of that one
// function inherited a hard dependency on the database env vars just to load. It now lives
// in `./nameMapping`; import it from there.
